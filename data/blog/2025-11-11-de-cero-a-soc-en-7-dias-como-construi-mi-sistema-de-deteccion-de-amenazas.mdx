---
title: 'De Cero a SOC en 7 Días: Cómo Construí mi Sistema de Detección de Amenazas'
date: '2025-11-11T00:00:00'
lastmod: '2025-11-11T00:00:00'
summary: 'Sprint profesional de 7 días: Wazuh sobre Proxmox y Docker, arquitectura LXC y smoke tests; proyecto orientado a empleabilidad y evidencias medibles.'
tags:
  [
    'proyecto',
    'tutorial',
...
> **Note:** Este es el **primer post** de la serie _SOAR Inteligente_ (Semana 1/8), planteado como un **sprint profesional**. El objetivo es demostrar **criterio de arquitectura, troubleshooting y métricas** que interesan a un hiring manager.
...
- **Métricas** verificables (500+ eventos/h, <10s latencia)
...
![Terminal mostrando error y solución](/static/images/soc-week01/error-agente-never-connected.png)
...
> **Tip:** Para _smoke tests_ reproducibles, guardá tus **consultas de Threat Hunting** (filtros por `rule.id`, `agent.name`, etc.).
...
![Múltiples tipos de alertas — SSH, FIM, sudo, HTTP](/static/images/soc-week01/alertas-multiples.png)
...
![Gráficos de performance: CPU, RAM, eventos/tiempo](/static/images/soc-week01/metricas-performance.png)
...
## Apéndice: Checklist de screenshots necesarios (calidad ≥1080p, PNG)

**CRÍTICOS (necesarios para MVP):**

1. ✅ Dashboard principal Wazuh mostrando eventos
2. ✅ Proxmox UI (LXC + VMs con recursos)
3. ✅ Alerta SSH (rule 5710) — LA MÁS IMPORTANTE
4. ✅ Terminal con error y solución
5. ✅ Métricas de performance (CPU, RAM, eventos)

**OPCIONALES (agregar después):** 6. Múltiples tipos de alertas (FIM, sudo, HTTP)  
7. Smoke test checkpoints (Docker ps, agent_control -l)

> **Tip:** Usá **ShareX / Flameshot / Greenshot** y anotaciones (flechas y círculos). La documentación clara es parte del valor del proyecto.
