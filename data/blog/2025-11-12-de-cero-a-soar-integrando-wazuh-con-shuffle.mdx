---
title: 'De Cero a SOAR: Desplegando Shuffle y conectándolo con Wazuh (Mi primera automatización real)'
date: '2025-11-12T00:00:00'
lastmod: '2025-11-12T00:00:00'
summary: 'De Wazuh solo a un SOAR operativo con Shuffle: arquitectura LXC en Proxmox, webhooks, errores reales y la primera alerta recibida en acción.'
tags:
...
> **Lesson Learned:** No todo proyecto necesita una VM completa. Si vas a orquestar con Docker, tener Docker _dentro_ de una VM es muchas veces una capa de abstracción innecesaria que solo suma complejidad.
...
1. **Create CT** (botón superior derecho)
2. **General:**
   - CT ID: `102`
   - Hostname: `shuffle-soar`
   - **Unprivileged container:** ❌ desmarcar (necesitamos privileged)
3. **Template:**
   - Storage: local
...
> **Note:** Usé `docker.io` del repositorio de Ubuntu por velocidad. Si necesitás Docker CE (Community Edition) oficial, seguí las [instrucciones de Docker](https://docs.docker.com/engine/install/ubuntu/).
...
- `BACKEND_HOSTNAME=shuffle-backend`: El frontend usa este nombre para llamar al backend **dentro de la red Docker**. Nunca pongas una IP externa acá.
...
1. **Workflows** (menú izquierdo) → **New Workflow**
2. **Name:** `Test-Wazuh-Connection`
3. **Description:** "Primer test de integración Wazuh→Shuffle"
4. Arrastrar nodo **Webhook** al canvas
5. Click en el nodo Webhook → **Show Execution Argument**
...
2. Usar `sed` o `echo` para inyectar config - más portable
...
- `<hook_url>`: La URL del webhook de Shuffle (¡reemplazá `XXXXX`!)
- `<level>3</level>`: Enviar alertas de nivel 3+ (severity: low, medium, high, critical)
- `<alert_format>json</alert_format>`: Formato JSON (más fácil de parsear en Shuffle)

> **Warning:** Cuidá la **indentación** en el `sed`. XML es sensible a espacios. Si copiás/pegás desde una terminal con encoding diferente, puede romper el archivo de config. Siempre verificá con `grep` después.
...
1. Dashboard → **Workflows** → `Test-Wazuh-Connection`
2. Tab **Executions** (arriba a la derecha)
3. Deberías ver una ejecución nueva con status "Success"
4. Click en la ejecución → **Execution Argument** muestra el JSON:
...
- **CPU usage:** <5% en idle, <20% procesando alertas
...
- Evento generado → Alerta en Shuffle: **<2 segundos**
...
1. **LXC > VM para servicios Docker en homelab**  
   Menor overhead, arranque instantáneo, mismo nivel de aislamiento para labs.

2. **Nombres de contenedor > IPs en Docker**  
   Docker DNS resuelve automáticamente. Hace el stack portable y fácil de reconfigurar.

3. **Webhooks > APIs para integraciones simples**  
   Para casos de "evento → acción", webhooks son más directos que polling de APIs.

4. **`sed` es tu amigo cuando no hay editores**  
   Aprendé los basics de `sed` para containers minimalistas. Ahorra tiempo.
...
2. **IP estática es crítica para integraciones**  
   DHCP puede cambiar IPs y romper webhooks. Siempre estático para servicios core.

3. **Privileged containers habilitan Docker-in-Docker**  
   Trade-off conocido: menos seguridad, más flexibilidad. Para labs está OK.
...
1. **Logs primero, siempre**  
   `docker-compose logs` te da el 80% de la info que necesitás para debuggear.

2. **Network issues = 90% de los problemas en Docker**  
   Cuando algo no funciona, empezá por verificar: nombres DNS, rutas, firewalls.

3. **Browser cache puede engañar**  
   Modo incógnito (Ctrl+Shift+N) o hard refresh (Ctrl+Shift+R) evita falsos negativos.
...
**Meta de Semana 2:**  
Bajar el **MTTR** (Mean Time To Response) de ~5 minutos (manual) a **<10 segundos** (automático).
...
- [Shuffle Docs](https://shuffler.io/docs) - Guías, ejemplos de workflows
- [Wazuh Integration Guide](https://documentation.wazuh.com/current/proof-of-concept-guide/integrate-external-tools.html) - Integración con herramientas externas
...
4. Click en Webhook → **Show Execution Argument**
...
**Próximo post:** "Playbook automático: de SSH brute force a IP bloqueada en <10 segundos"
